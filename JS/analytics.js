 const analyticsTransactions = JSON.parse(localStorage.getItem('transactions')) || [];

// ===== Stat Cards: Highest Expense, Highest Income, Avg Daily Spending =====
function renderAnalyticsStats() {
  const expenses = analyticsTransactions.filter(t => t.type === 'expense');
  const incomes = analyticsTransactions.filter(t => t.type === 'income');

  const highestExpense = expenses.length ? Math.max(...expenses.map(t => t.amount)) : 0;
  const highestIncome = incomes.length ? Math.max(...incomes.map(t => t.amount)) : 0;
  const totalExpense = expenses.reduce((a, b) => a + b.amount, 0);

  // Unique dates count karo average nikalne ke liye
  const uniqueDates = [...new Set(expenses.map(t => t.date))];
  const avgDaily = uniqueDates.length ? (totalExpense / uniqueDates.length) : 0;
  ...
  container.innerHTML = `
    <div class="stat-card">
      ...
      <div class="stat-info">
        ...
        <span class="stat-value expense-text">${formatAmount(highestExpense)}</span>
      </div>
    </div>
    <div class="stat-card">
      ...
      <div class="stat-info">
        ...
        <span class="stat-value income-text">${formatAmount(highestIncome)}</span>
      </div>
    </div>
    <div class="stat-card">
      ...
      <div class="stat-info">
        ...
        <span class="stat-value">${formatAmount(Math.round(avgDaily))}</span>
      </div>
    </div>
    <div class="stat-card">
      ...
      <div class="stat-info">
        ...
        <span class="stat-value">${analyticsTransactions.length}</span>
      </div>
    </div>
  `;
}
...
function renderAnalyticsCharts() {
  ...
  const income = analyticsTransactions.filter(t => t.type === 'income').reduce((a, b) => a + b.amount, 0);
  const expense = analyticsTransactions.filter(t => t.type === 'expense').reduce((a, b) => a + b.amount, 0);
